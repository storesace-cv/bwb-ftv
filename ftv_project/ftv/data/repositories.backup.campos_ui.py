...
class IngredientesRepo:
    ...
    def listar_por_produto(self, codigo: str):
        ...
        # fallback: caso nenhuma coluna seja detetada, devolve linhas "em branco" coerentes
        if not select_cols:
            try:
        ...
        try:
            cur.execute(f"SELECT {cols_sql} FROM fichas_tecnicas WHERE {prod_col} = ? ORDER BY ordem, rowid", (codigo,))
            rows = cur.fetchall()
            out = []
            for r in rows:
