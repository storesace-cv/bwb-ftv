...
class DataStore:
    ...
    def codigo_at(self, idx):
        ...
        if self.demo or not self.conn or getattr(self, 'ingredientes', None) is None:
            return []
    ...
    def _ensure_preparacao_table(self):
        ...
        if self.demo or not self.conn or getattr(self, 'prep', None) is None:
            return ""
        return self.prep.get_html(codigo)

        if self.demo or not self.conn or getattr(self, 'prep', None) is None:
            return
        return self.prep.upsert_html(codigo, html)
