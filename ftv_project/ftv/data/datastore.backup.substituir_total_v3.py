...
class DataStore:
    ...
    # Alergénios ativos (para grelha de alergénios na UI)
    def list_active_allergens(self):
        """
        Devolve lista de nomes de alergénios ativos.

        Ordem de tentativa:
          1) BD (tabela alergenios: id, nome, ativo)
          2) Ficheiro allergens.json na raiz do projeto (p.ex. { "alergenios": [ { "nome": ... }, ... ] })
        ...
        if self.conn and not self.demo:
            try:
                cur = self.conn.cursor()
                cur.execute("SELECT nome FROM alergenios WHERE ativo=1 ORDER BY nome")
                rows = cur.fetchall()
                names = []
                for r in rows or []:
                    try:
                        val = r["nome"] if hasattr(r, "keys") else r[0]
                    except Exception:
        ...
        if json_path.exists():
            try:
                ...
                # aceitar dois formatos: lista direta ou {"alergenios": [...]}
                ...
                if isinstance(payload, list):
                    for item in payload:
                        if isinstance(item, str):
                            ...
                            v = item.get("nome") or item.get("name")
