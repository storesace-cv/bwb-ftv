...
self._connect_nav()
    ...
    def _aux_fetch_lists(self):
        """Lê listas (id, nome) das 3 tabelas auxiliares diretamente da BD."""
        lists = {"tipo_artigo": [], "validade": [], "temperatura": []}
        try:
            cur = self.ds.conn.cursor()
            # Tipos de artigos
            try:
                cur.execute("SELECT id, descricao FROM tipos_artigos WHERE COALESCE(ativo,1)=1 ORDER BY descricao")
                lists["tipo_artigo"] = [(int(r[0]), str(r[1])) for r in cur.fetchall()]
            except Exception:
                lists["tipo_artigo"] = []
            # Validade
            try:
                cur.execute("SELECT id, descricao FROM validades WHERE COALESCE(ativo,1)=1 ORDER BY id, descricao")
                lists["validade"] = [(int(r[0]), str(r[1])) for r in cur.fetchall()]
            except Exception:
                lists["validade"] = []
            # Temperaturas
            try:
                cur.execute("SELECT id, descricao FROM temperaturas WHERE COALESCE(ativo,1)=1 ORDER BY id, descricao")
                lists["temperatura"] = [(int(r[0]), str(r[1])) for r in cur.fetchall()]
            except Exception:
                lists["temperatura"] = []
        except Exception as e:
            print(f"[AuxUI][ERRO] a ler listas: {e}")
        return lists
    ...
    def _aux_load_selected(self, codigo):
        ...
        if not any([cb_tipo, cb_val, cb_temp]):
            return
        # Lê produto_auxiliar; fallback produto_attrs
        ...
        def set_by_data(cb, wanted_id):
            ...
            try:
                if wanted_id is None:
                    ...
                    return
                # procura pelo itemData == wanted_id
    ...
    def _aux_wire_autosave(self):
        """Liga autosave aos 3 comboboxes, ignorando alterações durante _load_record."""
        cb_tipo, cb_val, cb_temp = self._aux_find_cbs()
        ...
        def saver(kind, cb):
            ...
            def handler(*_):
                ...
                if not codigo:
                    return
                # grava em produto_auxiliar
                col = {"tipo_artigo":"tipo_artigo_id","validade":"validade_id","temperatura":"temperatura_id"}[kind]
                try:
                    cur = self.ds.conn.cursor()
                    if value is None:
                        cur.execute(f"INSERT INTO produto_auxiliar (produto_codigo) VALUES (?) ON CONFLICT(produto_codigo) DO NOTHING", (codigo,))
                        ...
                        """, (codigo, int(value)))
                    self.ds.conn.commit()
                    # print(f"[AuxUI] gravado {kind}={value} para {codigo}")
                except Exception as e:
    ...
    def _aux_ensure_guard(self):
        ...
        def wrapped(idx:int):
            ...
            self._loading = True
            try:
                # primeiro popula (limpa lixo sempre que troca de produto)
                self._aux_populate_cbs()
                # chama original (preenche restantes campos)
                res = orig(idx)
                # depois posiciona os auxiliares segundo a BD
                try:
                    codigo = self.ds.codigo_at(getattr(self, "cur_index", 0))
                except Exception:
                    codigo = None
