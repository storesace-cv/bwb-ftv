...
def test_build_reportbro_context_uses_flat_currency_fields():
    ...
    payload["currency_symbol"] = "USD$"
    payload["currency_code"] = "USD"
    payload["locale_code"] = "en_US"

    dataset = build_reportbro_context(payload)

    assert dataset["currency_symbol"] == "USD$"
    assert dataset["currency_code"] == "USD"
    assert dataset["locale_code"] == "en_US"
    assert dataset["currency"]["currency_symbol"] == "USD$"
    assert "USD$" in dataset["PrecosTaxas_Preco1_display"]

    first_ingredient = dataset["ingredientes"][0]
    assert "USD$" in first_ingredient["FichasTecnicas_Ppu_display"]
    assert "USD$" in first_ingredient["FichasTecnicas_Preco_display"]
...
def test_build_reportbro_context_prefers_flat_currency_fields():
    ...
    payload["currency_symbol"] = "GBP£"
    payload["currency_code"] = "GBP"
    payload["locale_code"] = "en_GB"

    dataset = build_reportbro_context(payload)

    assert dataset["currency_symbol"] == "GBP£"
    assert dataset["currency_code"] == "GBP"
    assert dataset["locale_code"] == "en_GB"
    assert dataset["currency"]["currency_symbol"] == "GBP£"
    assert dataset["currency"]["currency_code"] == "GBP"

    preco_display = dataset["PrecosTaxas_Preco1_display"]
    assert "GBP£" in preco_display
    assert "€" not in preco_display

    first_ingredient = dataset["ingredientes"][0]
    assert "GBP£" in first_ingredient["FichasTecnicas_Ppu_display"]
    assert "GBP£" in first_ingredient["FichasTecnicas_Preco_display"]
