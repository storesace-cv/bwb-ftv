...
| **Editor de Documentos** | Acesso ao editor ReportBro para criar e ajustar modelos de impressão. | Menu "Menu → Utilitários → Gestão de Documentos → Editor de Documentos" na aplicação; abre `FTV_REPORTBRO_EDITOR_URL` ou, se não configurado, o editor offline incluído. | Equipa de UI e responsáveis pelas integrações de impressão.
