...
| **Gestor de Documentos** | Acesso ao editor ReportBro para criar e ajustar modelos de impressão. | Menu "Menu → Utilitários → Gestor de Documentos" na aplicação; abre `FTV_REPORTBRO_EDITOR_URL` (ou o editor público por omissão). | Equipa de UI e responsáveis pelas integrações de impressão.
